  ...
  module PhotographerDirectory
    ...
    class ProfileWriter
      ...
      def update!(attrs)
        ...
        raise UniqueConflict, friendly_unique_message(e)
      rescue ActiveRecord::InvalidForeignKey => e
        raise ForeignKeyMissing, friendly_fk_message(e)
      rescue ActiveRecord::StatementInvalid => e
        raise GrantsMissing, "directory DB rejected the write — most likely missing column UPDATE grants. Original: #{e.message.truncate(180)}" if e.message.include?('permission denied')
        raise UniqueConflict, friendly_unique_message(e) if e.message.include?('duplicate key value violates unique constraint')
        raise ForeignKeyMissing, friendly_fk_message(e) if e.message.include?('foreign key constraint')
      ...
      def validate!(attrs)
        ...
        if attrs.key?(:country_code)
          cc = attrs[:country_code].to_s
          raise ValidationError, "country_code must be a 2-letter ISO code (got '#{cc}')" if cc.length != 2 && !cc.empty?
        end
